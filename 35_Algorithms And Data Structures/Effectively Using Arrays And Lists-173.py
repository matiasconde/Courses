...
for number in numbers: 
    binary.append(bin(number).replace("0b",""))
    
for number in binary:
    new_numbers.append(int(number,2))
    
check = True
for i in range(100):
    if numbers[i] == new_numbers[i]:
        check &= True
    else: 
        check &= False
...
values_elements_hex = [hex(id(e)) for e in values]
...
all_prices = [float(p.strip()) for p in all_prices] # strip le saca el primer espacio.
...
class Node():
    ...
    def pop(self,position):
        ...
        if split_end == None: return split_start, self
