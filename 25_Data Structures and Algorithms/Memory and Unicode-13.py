...
def clean_data(row):
    ...
    data = row["statement"]
    cleaned_data = [s for s in data if ord(s) in good_characters]
    return "".join(cleaned_data)

sentences_cia["cleaned_statement"] = sentences_cia.apply(clean_data,axis=1)
...
def most_common_term_by_year(data,string_year):
    data_year = data.loc[data["year"]==string_year]
    data_year_combined = " ".join(data_year["cleaned_statement"])
    words = data_year_combined.split(" ")
    words_relevant = [s for s in words if len(s)>=5]
    frequencies = Counter(words_relevant)
    return frequencies.most_common(2)

common_2000 = most_common_term_by_year(sentences_cia,"2000")
common_2002 = most_common_term_by_year(sentences_cia,"2002")
common_2013 = most_common_term_by_year(sentences_cia,"2013")
