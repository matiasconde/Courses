...
print(type(sex_pivot))
print(sex_pivot)

pcclass_pivot = train.pivot_table(index="Pclass",values="Survived")
pcclass_pivot.plot.bar()
plt.show()
...
train_X,test_X,train_y,test_y = train_test_split(train[columns],train["Survived"],random_state=0,test_size=0.2)
...
lr = LogisticRegression()
scores = cross_val_score(estimator=lr,X=train[columns],y=train["Survived"],cv=10)
