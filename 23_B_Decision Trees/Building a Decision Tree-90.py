...
def id3(data, target, columns):
    ...
    if len(unique_targets) == 1:
        # Insert code here to append 1 to label_1s or 0 to label_0s, based on what we should label the nodev
        value = unique_targets[0]

        if value==1:
            label_1s.append(1)
        else: label_0s.append(0)
...
def predict(tree, row):
    ...
    else: return predict(tree["right"],row)
...
def batch_predict(tree, df):
    ...
    df2 = df.apply(lambda x: predict(tree,x),axis=1)
    return df2
