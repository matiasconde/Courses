...
with open("ign.csv",'r') as f:
    reader = csv.reader(f) # agarramos el reader
    next(reader)  # esto es para que no tome la primer linea
    rows = [row for row in reader] #metemos todas las filas en una lista de filas. 
    

for row in rows: 
    cur.execute("INSERT INTO ign_reviews VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",row) # usamos la opción 2 para insertar cada fila (más rápida y performante). 
conn.commit() 
...
conn = psycopg2.connect("dbname=dq user=dq")
cur = conn.cursor()
cur.execute("""INSERT INTO ign_reviews (id, score_phrase, title, url, platform, score, genre, editors_choice, release_date) SELECT id, score_phrase, title_of_game_review, url, platform, score, genre, editors_choice, to_date(release_day || '-' || release_month || '-' || release_year, 'DD-MM-YYYY') FROM old_ign_reviews""")
conn.commit()
